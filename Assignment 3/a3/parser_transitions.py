...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses[:]
    for parser in unfinished_parses:
        if len(parser.stack) == 1 and len(parser.buffer) == 0:
            unfinished_parses.remove(parser)

    while len(unfinished_parses) > 0:
        right = batch_size if batch_size < len(unfinished_parses) else len(unfinished_parses)
        transitions = model.predict(unfinished_parses[0: right])
        for i in range(right):
            unfinished_parses[i].parse_step(transitions[i])
        for parser in unfinished_parses[0:right]:
            if len(parser.stack) == 1 and len(parser.buffer) == 0:
                unfinished_parses.remove(parser)
    ...
    #
    #     left = i * batch_size
    #     if left > len(sentences) - 1:
    #         break
    #     if i * batch_size + batch_size > len(sentences) -1:
    #         right = len(sentences) - 1
    #     else:
    #         right = i * batch_size + batch_size
    #     transitions += model.predict(sentences[left, right])
