...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for i, sent in enumerate(sents):
            ...
            for j, word in enumerate(sent):
                word_ids[i].append([])
                word_ids[i][j].append(self.start_of_word)
                for char in word:
                    word_ids[i][j].append(self.char2id[char])
                word_ids[i][j].append(self.end_of_word)
        ### END YOUR CODE
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        return torch.tensor(pad_sents_char(self.words2charindices(sents), self.char2id['<pad>']),
                            device=device).long().permute((1, 0, 2))
